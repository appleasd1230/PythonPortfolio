...
def getNewsList(state):
	...
	for i in range(0,len(press)):
		try:
			if state == 'new':
				newThread = getURLThread(pressAbbr[i]+'Thread', spiders[i])#Threading.py
...
def getContent(state, NewsLists, record):
	...
	for NewsList in NewsLists:
		...
		try:
			if state == 'new':
				newThread = getContentThread(pressAbbr[index]+'Thread', spiders[index], NewsList['URLList'], record)#Threading.py
			elif state == 'check':
