	...
	def getContent(self, ARTICLE_List, record):
		...
		for articleURL in ARTICLE_List:
			...
			if t.strftime('%Y/%m/%d', yesterday) not in datetime:
				...
				pass
			#get content
			content_lsit = news.find('p', {'id':'summary'}).findAll(text=True)
			if content_lsit:
				article = news.find('p', {'id':'summary'}).findAll(text=True)
			else:
				article = news.find('p', {'id':'caption1'}).findAll(text=True)
	...
	def getURL(self):
		...
		for URL in self.URLList:
			...
			for article in articles:
				...
				if 'realtimenews' not in articleURL:
					continue
				self.ARTICLE_List.append(articleURL)
	...
	def getContent(self, ARTICLE_List, record):
		...
		for articleURL in ARTICLE_List:
			...
			try:
				...
				time = news.find('div', {'class':'story_bady_info_author'}).span.text
				time = re.split('-| |:', time)
				datetime = '/'.join(time[:3])
				timeInNews = ':'.join(time[3:])
				article = news.findAll('p')
