...
class getURLThread(threading.Thread):
	...
	def run(self):
		...
		self.newsList = self.spider.getURL()#NewsSpider.py、NewsSpiderCheck.py
		time.sleep(1)
...
class getContentThread(threading.Thread):
	...
	def run(self):
		print ('    Starting ' + self.name + time.strftime('  %Y/%m/%d %H:%M:%S', time.localtime()))
		self.newsList = self.spider.getContent(self.URLList, self.record)#NewsSpider.py、NewsSpiderCheck.py
		time.sleep(1)
